 ...
 * AliVcMediaPlayerbackStateDidChangeNotification:播放器状态改变后发送该通知，收到此通知，可以改变UI状态，更新相关显示等。
 ...
 *   1.	验证是否有权限调用该函数,无权限则返回ALIVC_ERR_FUNCTION_DENIED。
     ...
     3.	其他情况则停止视频播放，并将播放器状态设置为Stopped，发送播放器状态改变通知AliVcMediaPlayerbackStateDidChangeNotification，视频停止后会发送视频结束通知AliVcMediaPlayerPlaybackDidFinishNotification。
